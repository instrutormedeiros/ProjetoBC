...
function setupProtection() {
    ...
    // Proteções DESATIVADAS para permitir F12 e debugging
    // document.addEventListener('contextmenu', e => e.preventDefault());
    // document.addEventListener('keydown', e => {
    //     if (e.ctrlKey || e.metaKey) {
    //         if (['c','a','x','v','s','p','u'].includes(e.key.toLowerCase()))
    //             e.preventDefault();
    //     }
...
// --- Auto-init in case access overlay was hidden before init was defined --- 
(function(){
  try {
    if (typeof init === 'function') {
      ...
      setTimeout(() => {
        ...
        // if overlay element not present or explicitly hidden, initialize the app
        if (!ov || ov.style.display === 'none' || ov.classList.contains('hidden')) {
          init();
        }
      }, 200);
    }
    if (typeof init === 'function' && typeof window.initCourse === 'undefined') {
